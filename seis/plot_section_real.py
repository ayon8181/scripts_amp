...
for ev in ev_list:
    ds=ASDFDataSet("/scratch1/09038/ayon8181/pypaw_workflow_test/seis/proc/"+ev+".T017-050s.proc_real_data.h5",mode="r")
    ...
    for st in stations:
        ...
        if stla==stla and stlo==stlo:
            ...
            if dist is not np.nan and arrival is not np.nan:
                try:
                   data=ds.waveforms[st].proc_real_data.select(component="T")[0]
                ...
                """
                if ph==0:
                plt.scatter(arrival,dist,marker="|",color="red",label="S Phase")
                k=model.get_ray_paths_geo(evdp,evla,evlo,stla,stlo,['SS'])
                if len(k)!=0:
                    p_arr=k[0].time
                    plt.scatter(arrival,dist,marker="|",color="blue",label="SS Phase")
                k=model.get_ray_paths_geo(evdp,evla,evlo,stla,stlo,['SSS'])
                if len(k)!=0:
                    p_arr=k[0].time
                    plt.scatter(arrival,dist,marker="|",color="gold",label="SSS Phase")
                k=model.get_ray_paths_geo(evdp,evla,evlo,stla,stlo,['ScS']) 
                if len(k)!=0:
                    p_arr=k[0].time
                    plt.scatter(arrival,dist,marker="|",color="skyblue",label="ScS Phase") 
                k=model.get_ray_paths_geo(evdp,evla,evlo,stla,stlo,['Sdiff']) 
                if len(k)!=0:
                    p_arr=k[0].time
                    plt.scatter(arrival,dist,marker="|",color="green",label="Sdiff Phase")
                if ph==1:
                plt.scatter(arrival,dist,marker="|",color="green",label="Sdiff Phase")
                k=model.get_ray_paths_geo(evdp,evla,evlo,stla,stlo,['SS'])
                if len(k)!=0:
    ...
    plt.savefig("/scratch1/09038/ayon8181/pypaw_workflow_test/outputs/sections/"+ev+"_real_section.png")
