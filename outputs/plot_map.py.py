...
for k,ph in enumerate(phase_list):
    for i,dfs in enumerate(df):
        ...
        if k==0:
           df_plot = df_plot[(df_plot[6]>30) & (df_plot[6]<70)][0,1,2,3,4,23+k*5+3,23+k*5+4,23+k*5+2]
           pool = mp.Pool(int(os.environ['SLURM_CPUS_PER_TASK']))
           results=pool.starmap(points_S_ScS, [(rows[2],rows[1],rows[3],rows[5],rows[4],model,['S'])  for i,rows in dfs.iterrows()])
           pool.close()
           pool.join()
           start=[]
           mid=[]
           ...
           df_plot = df_plot[(df_plot[6]>50) & (df_plot[6]<140)][0,1,2,3,4,23+k*5+3,23+k*5+4,23+k*5+2]
           pool = mp.Pool(int(os.environ['SLURM_CPUS_PER_TASK']))
           results=pool.starmap(points_SS, [(rows[2],rows[1],rows[3],rows[5],rows[4],model)  for i,rows in dfs.iterrows()])
           pool.close()
           pool.join()
           start=[]
           mid=[]
           ...
           df_plot = df_plot[((df_plot[6]>75) & (df_plot[6]<110)) | ((df_plot[6]>110) & (df_plot[6]<165))][0,1,2,3,4,23+k*5+3,23+k*5+4,23+k*5+2]
           pool = mp.Pool(int(os.environ['SLURM_CPUS_PER_TASK']))
           results=pool.starmap(points_SSS, [(rows[2],rows[1],rows[3],rows[5],rows[4],model)  for i,rows in dfs.iterrows()])
           pool.close()
           pool.join()
           start=[]
           mid=[]
           ...
           df_plot = df_plot[((df_plot[6]>5) & (df_plot[6]<25)) | ((df_plot[6]>50) & (df_plot[6]<65)) ][0,1,2,3,4,23+k*5+3,23+k*5+4,23+k*5+2]
           pool = mp.Pool(int(os.environ['SLURM_CPUS_PER_TASK']))
           results=pool.starmap(points_S_ScS, [(rows[2],rows[1],rows[3],rows[5],rows[4],model,['ScS'])  for i,rows in dfs.iterrows()])
           pool.close()
           pool.join()
           start=[]
           mid=[]
           ...
           df_plot = df_plot[(df_plot[6]>100) & (df_plot[6]<150)][0,1,2,3,4,23+k*5+3,23+k*5+4,23+k*5+2]
           pool = mp.Pool(int(os.environ['SLURM_CPUS_PER_TASK']))
           results=pool.starmap(points_Sdiff, [(rows[2],rows[1],rows[3],rows[5],rows[4],model)  for i,rows in dfs.iterrows()])
           pool.close()
           pool.join()
           start=[]
           mid=[]
        ...
        fig.plot(data=df_plot.mid,style="c0.05c",fill=df_plot[23+k*5+3],cmap=True,transparency=40)
        fig.plot(data=df_plot.start,style="=0.5c",fill=df_plot[23+k*5+3],cmap=True,transparency=40)            
        fig.plot(data=df_plot.end,style="=0.5c",fill=df_plot[23+k*5+3],cmap=True,transparency=40)
        fig.colorbar()
        ...
        fig.basemap(region="d",projection="N-150/12c")
        fig.coast(shorelines=True, frame=True)   
        fig.plot(data=df_plot.mid,style="c0.05c",fill=df_plot[23+k*5+4],cmap=True,transparency=40)
        fig.plot(data=df_plot.start,style="=0.5c",fill=df_plot[23+k*5+4],cmap=True,transparency=40)            
        fig.plot(data=df_plot.end,style="=0.5c",fill=df_plot[23+k*5+4],cmap=True,transparency=40)
        fig.colorbar()
        ...
        fig.basemap(region="d",projection="N-150/12c")
        fig.coast(shorelines=True, frame=True)   
        fig.plot(data=df_plot.mid,style="c0.05c",fill=df_plot[23+k*5+2],cmap=True,transparency=40)
        fig.plot(data=df_plot.start,style="=0.5c",fill=df_plot[23+k*5+2],cmap=True,transparency=40)            
        fig.plot(data=df_plot.end,style="=0.5c",fill=df_plot[23+k*5+2],cmap=True,transparency=40)
        fig.colorbar()
