...
file_names=["prem_3D_crust.txt","obsd_3D_crust.txt","obsd_glad.txt","real_data.txt","obsd_1D_crust.txt","obsd_3D_crust_15.txt","obsd_3D_crust_2.txt","prem_3D_atten.txt","prem_q16.txt","obsd_3D_crust_atten.txt"]

#file_names=["obsd_glad_real.txt","obsd_3D_crust_real.txt","prem_3D_crust_real.txt","1D_ref_real.txt","prem_q16_real.txt","prem_3D_atten_real.txt","synt_real.txt","obsd_3D_crust_atten_real.txt","obsd_1D_crust_real.txt","obsd_3D_crust_2_real.txt","obsd_3D_crust_15_real.txt"]
...
d_all=pd.merge(df[0],df[1],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_prem_3D","_3D"))
d_all=pd.merge(d_all,df[2],how='left',on=[0,1,2,3,4,5,6,7])
d_all=pd.merge(d_all,df[3],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_glad","_real"))
d_all=pd.merge(d_all,df[4],how='left',on=[0,1,2,3,4,5,6,7])
d_all=pd.merge(d_all,df[5],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_1D","_3D_5"))
d_all=pd.merge(d_all,df[6],how='left',on=[0,1,2,3,4,5,6,7])
d_all=pd.merge(d_all,df[7],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_3D_11","_3D_atten"))
d_all=pd.merge(d_all,df[8],how='left',on=[0,1],suffixes=(None,"_prem_16"))
d_all=pd.merge(d_all,df[9],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_prem_16","_S80_3D"))
#d_all=pd.merge(d_all,df[10],how='left',on=[0,1,2,3,4,5,6,7])
#d_all=pd.merge(d_all,df[10],how='left',on=[0,1,2,3,4,5,6,7],suffixes=("_1D_ref","_fow"))
#d_all.rename(columns={'7_prem_3D': 7}, inplace=True)



gsn=["II","IU","GT","IC","CU"]
phase_list=['S','SS','SSS','ScS','Sdiff']
names=["_prem_3D","_3D","_glad","_real","_1D","_3D_11","_3D_5","_3D_atten","_prem_16","_S80_3D"]
for k,ph in enumerate(phase_list):
    print(d_all[str(18+k*11+i)+"_prem_16"])
    col_list=[]
    for i in range(8):
        col_list.append(i)
    for n in names:
...
"""
df_out=d_all[col_list]
df_out.to_csv("/scratch1/09038/ayon8181/scripts_amp/outputs/SS_Sdiff_all.txt",index=False,na_rep=np.nan)
...
"""
df_out=d_all[col_list]
df_out.to_csv("/scratch1/09038/ayon8181/scripts_amp/outputs/SSS_Sdiff_all.txt",index=False,na_rep=np.nan)
