...
plot_dir="/scratch1/09038/ayon8181/scripts_amp/outputs/today2"
...
# k=[4]
df_plot =pd.read_csv("/scratch1/09038/ayon8181/scripts_amp/outputs/SSS_all_real.txt",delimiter=",",skipinitialspace=True)
lws=[1,1,1,1,1,2]
...
labels=["real_data","PREM","S40RTS_Crust2.0","GLAD_M25"]#"GLAD_M25","PREM_Crust2.0"] 
colors=['black','red','deepskyblue','green','forestgreen','indianred','grey','peru']
colors=["black","royalblue","red","forestgreen","red"]
phase=['Sdiff']

names=["proc_real_data","proc_obsd_3D_crust_atten","proc_obsd_3D_crust"]#,"proc_obsd_glad"]#,"proc_prem_3D_atten"]#,"proc_prem_3D_crust","proc_obsd_glad","proc_real_data"]
tags =["proc_real_data","proc_obsd_33","proc_obsd_3"]#,"proc_obsd_25"]#
names2=["_real","_S80_3D","_3D"]
lws=[3,1.5,3,1.5]
def plot(event,df_plot,station,t_min,t_max,ph):
    ...
    for l,d in enumerate(names):
        ...
        if station in ds_synt.waveforms.list() and len(ds_synt.waveforms[station][tags[l]].select(component="T"))>0:
            ...
            print(stla,stlo,station)
        
            
            arrival_time=model.get_travel_times_geo(evdp,evla,evlo,stla,stlo,phase_list=[ph])[0].time
            #except IndexError:
            ...
            if l==0:
                ...
                ax1=fig.add_subplot(1,1,1) 
                        #print("ax1 created")
                ax1.plot(time_series,data_wdow,color=colors[l], lw=lws[l], label=labels[l])
            ...
            if l!=0:
                ...
                if 2==2:
                    ...
                    #arr_index     = int((arrival_time-df_event[str(18+4*11+2)+names2[l]].iloc[0])*sampling_rate)
                    #print(arr_index)
                    arr_index       = int(arrival_time*sampling_rate)
...
stations=[["GT.LBTB","PR.HUMP","C1.ML02","N4.Y49A","DK.KULLO"]]
df_plot =pd.read_csv("/scratch1/09038/ayon8181/scripts_amp/outputs/SSS_all_real.txt",delimiter=",",skipinitialspace=True)
